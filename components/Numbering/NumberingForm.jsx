...
const items = ["prefix", "group", "subgroup", "year", "month", "day", "number_digits", "suffix"];
...
export default function NumberingForm({ document, onClose }) {
    ...
    const generatePreviewCode = () => {
        const {
            prefix, suffix, number_digits, start_number, year, month, day, group, subgroup
        } = form;
        ...
        const code = formatCode.map((item) => {
            switch (item) {
                ...
                case "number_digits": return numberValue;
    ...
    const handleSubmit = async () => {
        ...
        // ตรวจสอบค่า group และ subgroup
        if (openGroup && form.group.some((g) => g.value.trim() === "")) {
            toast.error(lang["group_value_required"]);
            return;
        }
    
        if (openSubGroup && form.subgroup.some((sg) => sg.value.trim() === "")) {
            toast.error(lang["subgroup_value_required"]);
            return;
        }
    ...
    return (
        <div className="bg-white dark:bg-gray-800 w-[600px] h-full">
            <div className="flex flex-row justify-between items-center bg-orange-500 text-white p-2 w-full">
                ...
                <button type="button" onClick={onClose} className="text-white hover:text-gray-500">
                    <IoClose size={20} />
                </button>
            </div>
            ...
            <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                <SortableContext items={formatCode} strategy={verticalListSortingStrategy}>
                    <div className="flex flex-col bg-gray-100 px-4 pt-2 pb-4 gap-4">
                        ...
                        <div className="flex flex-wrap gap-2 bg-gray-100 rounded-md">
                            {formatCode.map((item) => (
                                <SortableItem key={item} id={item} onContextMenu={handleContextMenu} />
                            ))}
                        </div>
                    </div>
                </SortableContext>
            ...
            {contextMenu && (
                ...
                >
                    <button className="block px-2 py-1 text-red-500 hover:bg-gray-200" onClick={() => removeFormatItem(contextMenu.item)}>
                        ลบ {contextMenu.item}
                    </button>
                </div>
            )}
                ...
                <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {removedItems.map((item) => (
                            <button key={item} className="px-2 py-1 bg-blue-500 text-white rounded-md hover:bg-blue-600" onClick={() => addFormatItem(item)}>
                                + {item}
                            </button>
                        ))}
                    </div>
            ...
            <div className="flex flex-col px-4 py-2 gap-2">
                ...
                <div className="flex flex-row items-center gap-2">
                    ...
                    <input 
                        ...
                        onChange={(e) => setForm({ ...form, number_digits: e.target.value})}
                    />
                    <span className="text-xs text-red-500">({lang["number_digits_placeholder"]})</span>
                </div>
                ...
                <div className="flex flex-row items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-red-500">({lang["start_number_placeholder"]})</span>
                </div>
                                ...
                                groups.map((group, index) => (
                                    <div key={index} className="flex flex-row items-center gap-2">
                                        ...
                                        <input
                                            type="text"
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-20 p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                            value={form.group.find((g) => g.name === group.name)?.value || ""}
                                            onChange={(e) => handleChange("group", e.target.value, group.name)}
                                        />
                                    </div>
                                ))
                            ) : (
                                ...
                                subGroups.map((subgroup, index) => (
                                    <div key={index} className="flex flex-row items-center gap-2">
                                        ...
                                        <input
                                            type="text"
                                            className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-20 p-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
                                            value={form.subgroup.find((sg) => sg.name === subgroup.name)?.value || ""}
                                            onChange={(e) => handleChange("subgroup", e.target.value, subgroup.name)}
                                        />
                                    </div>
                                ))
                            ) : (
