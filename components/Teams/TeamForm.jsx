...
export default function TeamForm({ teamData, onClose }) {
  ...
  useEffect(() => {
    if (team.name.en) {
      setSlug(generateSlug(team.name.en));
    }
  }, [team.name.en]);
  ...
  const generateSlug = (text) => {
    return text
      .trim()
      .toLowerCase()
      .replace(/\([^)]*\)/g, "") // ลบข้อความที่อยู่ในวงเล็บ ()
      .replace(/[^\w\s-]/g, "") // ลบอักขระพิเศษที่ไม่ใช่ตัวอักษร, ตัวเลข, หรือช่องว่าง
      .replace(/\s+/g, "-") // แปลงช่องว่างเป็น `-`
      .replace(/-+/g, "-") // ป้องกันเครื่องหมาย `-` ซ้อนกัน
      .replace(/^-+|-+$/g, ""); // ลบ `-` ที่ต้นหรือท้ายข้อความ
  };
    ...
    <div className="flex flex-col p-2 bg-white dark:bg-gray-800 text-black dark:text-gray-50 min-w-[500px] w-full h-full">
      ...
      <div className="flex flex-col p-4 gap-2 w-full">
        <div className="flex flex-col justify-center items-center gap-2">
          ...
          <UploadImage onUpload={handleUploadImage} folder="teams" size={30} />
        </div>
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            value={team.name.en}
            onChange={(e) =>
              setTeam({ ...team, name: { ...team.name, en: e.target.value } })
            }
          />
          <input
            type="text"
            ...
            value={team.name.th}
            onChange={(e) =>
              setTeam({ ...team, name: { ...team.name, th: e.target.value } })
            }
          />
        </div>
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            onChange={(e) =>
              setTeam({
                ...team,
                position: { ...team.position, en: e.target.value },
              })
            }
          />
          <input
            ...
            onChange={(e) =>
              setTeam({
                ...team,
                position: { ...team.position, th: e.target.value },
              })
            }
          />
        </div>
        ...
        <div className="flex flex-col">
          ...
          <input
            ...
            value={team.bio.en}
            onChange={(e) =>
              setTeam({ ...team, bio: { ...team.bio, en: e.target.value } })
            }
          />
          <input
            type="text"
            ...
            value={team.bio.th}
            onChange={(e) =>
              setTeam({ ...team, bio: { ...team.bio, th: e.target.value } })
            }
          />
        </div>
      </div>
