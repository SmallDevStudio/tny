    ...
    return (
        <div className="relative inline-block">
            <Tooltip title={lang["changelanguage"]} placement="bottom">
                ...
                >
                    <Image src={selectedFlag?.flag} alt={selectedFlag?.name} width={20} height={20} className={isText ? "mr-2" : ""} />
                    {isText ? <span>{selectedFlag?.name}</span> : <span className="ml-2 uppercase">{selectedFlag?.code}</span>}
            ...
            {isOpen && (
                <ul className="absolute left-0 mt-2 w-32 bg-white border border-gray-300 rounded-md shadow-lg z-10">
                    {flag.map((item) => (
                        <li key={item.code} className="flex items-center gap-2 px-3 py-2 hover:bg-gray-100 cursor-pointer"
                            onClick={() => {
                                dispatch(changeLanguage(item.code));
                                setIsOpen(false);
                            }}>
                            <Image src={item.flag} alt={item.name} width={20} height={20} />
