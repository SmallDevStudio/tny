  ...
  return cart.length > 0 ? (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
      <div className="flex flex-col justify-between">
        <div>
          <ul className="overflow-y-auto space-y-2 list-disc">
            {cart.map((item) => (
              <>
                <li
                  key={item.courseId}
                ...
                >
                  <div className="flex-1">
                    <p className="font-medium text-gray-700 dark:text-gray-100">
                      {t(item.title)} {item.type && `(${item.type})`}
                    </p>
                ...
                <Divider flexItem sx={{ my: 1 }} key={item.courseId} />
              </>
            ))}
          </ul>
        </div>
      ...
      <div className="border border-gray-300 rounded-lg p-4 bg-white shadow-sm">
        ...
        <div className="flex justify-center">
          <button
            onClick={() => router.push("/payments/customerdetails")}
            className={
              cart.length === 0
