...
export default function ShareOn() {
    return (
        <div className="hidden lg:fixed lg:top-1/2 lg:-translate-y-1/2 md:flex gap-2 px-2 py-4 bg-gray-100 rounded-e-xl dark:bg-gray-900">
            <div className="flex flex-col items-center gap-4">
            <h1 className="font-bold text-xs text-orange-500 dark:text-gray-200">Share On</h1>
                {socialData.map((item, index) => (
                    <div key={index} className="flex items-center gap-2">
                        <Link href={item.url ? item.url : `#`} target="_blank" rel="noopener noreferrer"
                            className="text-orange-500 hover:text-gray-300 dark:text-gray-200 dark:hover:text-orange-500"
                        >
                        {item.icon}
                        </Link>
                    </div>
